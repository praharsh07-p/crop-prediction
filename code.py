...
def analyse():
	if(request.method == "POST"):
		...
		optimum['Ni'] = optimum.Ni.astype(float)
		optimum['Zn'] = optimum.Zn.astype(float)
		optimum['S'] = optimum.S.astype(float)
		optimum['Mg'] = optimum.Mg.astype(float)
		optimum['B'] = optimum.B.astype(float)
		optimum['Cl'] = optimum.Cl.astype(float)
		optimum['Mn'] = optimum.Mn.astype(float)
		optimum['Mo'] = optimum.Mo.astype(float)
		optimum['Ca'] = optimum.Ca.astype(float)
		optimum['Cu'] = optimum.Cu.astype(float)
		optimum['Fe'] = optimum.Fe.astype(float)
		...
		if(request.form.get('Potassium') == None):
			...
			columns = ['N','P','K','Ni','Zn','S','Mg','B','Cl','Mn','Mo','Ca','Cu','Fe','pH','TEMPERATURE'] 
			values = np.array([ nitrogen ,phosphorous ,potassium ,Nickel ,zinc ,Sulphur,Magnesium,Boron,Chlorine,Manganese,Molybdenum,Calcium,Copper,Iron,pH ,temperature])
